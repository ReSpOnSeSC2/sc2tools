...
Public YouTube recordings have a shared `public_youtube_archives` cache. When a requesting user has an existing YouTube connection, one bounded Data API `videos.list` request verifies up to 30 discovered video IDs, public privacy, channel ownership, and actual broadcast start/end times. The cache contains only public video IDs, channel IDs, and timing/orientation metadata; it never stores the requesting user's identity, credentials, or private/unlisted videos. Other users reuse that snapshot without needing a YouTube connection. Snapshots refresh after five minutes (30 seconds for ongoing streams), survive temporary provider failures for at most 24 hours, and expire through a MongoDB TTL index. Missing connections or failed official lookups retain the public-page fallback. HTTP 429 pauses public scraping and preserves the last verified results instead of issuing the remaining watch requests. A channel not yet indexed still needs either a working public-page lookup or a viewer with a connected YouTube account to populate its official snapshot.
