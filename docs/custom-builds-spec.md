...
### Discovery engine (`/v1/auto-classify/*`, served by `apps/api`)

The auto-classifier surfaces lookalike groups of *unclassified* games
(catch-all labels like `Macro Transition (Unclassified)`, empty
`myBuild`, etc.) and lets the user promote those groups into real
custom builds in one click. Both endpoints require a Clerk JWT.

#### `GET /v1/auto-classify/candidates`

Dry-run discovery. **Never writes to Mongo.** Walks the user's recent
games via `PerGameComputeService.listForRulePreview`, filters to the
unclassified bucket (anything not already claimed by a named detector
or an existing user custom build), clusters the survivors per matchup,
and returns a sorted candidate list.

Query parameters:

| Param         | Values                                          | Default |
|---------------|-------------------------------------------------|---------|
| `perspective` | `you` (cluster on user events) or `opponent`    | `you`   |
  ...
  "candidates": [
    {
      "matchup": "PvZ",
      "perspective": "you",
      "proposedName": "PvZ - Stargate into Tempest",
      "proposedSlug": "pvz-stargate-into-tempest",
      "race": "Protoss",          // Protoss | Terran | Zerg
      "vsRace": "Zerg",           // Protoss | Terran | Zerg
      "rules": [
        { "type": "before", "name": "BuildStargate", "time_lt": 240 }
      ],
      "gameCount": 7,             // games in the cluster (>= 5)
      "winRate": 0.57,            // 0..1
      "cohesion": 0.91,           // mean intra-cluster signature similarity
      "selfMatchRate": 1.0,       // 0..1; fraction of cluster games passing the derived rules
      "sampleGames": [
        { "gameId": "...", "map": "Equilibrium LE", "result": "Victory", "date": "..." }
      ]
    }
  ]
}
...
Candidates are sorted by `gameCount` desc, then `cohesion` desc, then
`matchup` alphabetically, then `proposedSlug` alphabetically. A name
that collides with an existing build (or a non-unclassified `myBuild`
label) automatically gets a deterministic `" (Auto)"` / `"-auto"`
suffix — the orchestrator never returns a colliding name.

Cached for 60s per `(userId, perspective, latestGameDateMs)`; a fresh
upload blows the cache key without waiting for the TTL.

#### `POST /v1/auto-classify/apply`

Promote one or more candidates into real custom builds. Each candidate
is validated against the same schema hand-authored builds pass through
(`apps/api/src/validation/customBuild.js`) and rejected as a batch if
any row fails — no partial-garbage writes.

After upsert, runs `CustomBuildsService.reclassifyAll` with
`clearUnmatched: false` so the global *most-specific-build wins* rule
applies. A freshly-created auto build can never steal a tag from a
more-specific existing build, and tags owned by the agent's named
catalog are left untouched.

Each persisted document carries `source: "auto-classify"` for
provenance. Read paths (list / stats / publish / community) never
gate on this field — an auto build is a first-class custom build in
every other respect.
...
{
  "candidates": [
    // Each entry MUST carry the fields below. Any extra discovery
    // metadata (gameCount, cohesion, sampleGames…) is ignored — it
    // never reaches the persisted build document.
    {
      "proposedName": "PvZ - Stargate into Tempest",
      "proposedSlug": "pvz-stargate-into-tempest",
      "race": "Protoss",
...
Max 50 candidates per call (the route returns `400 bad_request` if you
exceed it). 400 paths:

| Body                                | Response                                      |
|-------------------------------------|-----------------------------------------------|
| `candidates` missing / not an array | `400 bad_request`                             |
| `candidates` empty                  | `400 bad_request`                             |
| `candidates.length > 50`            | `400 bad_request`                             |
| any candidate fails validation      | `400 invalid_candidates` with `details[]` per row |
...
{
  "ok": true,
  "created": ["pvz-stargate-into-tempest"],   // slugs of upserted builds
  "builds": 3,                                 // total saved builds the reclassify pass scanned
  "scanned": 240,
  "tagged": 7,                                 // games whose myBuild changed
  "cleared": 0,
  "perBuild": [
    { "slug": "pvz-stargate-into-tempest", "name": "PvZ - Stargate into Tempest", "matched": 7, "tagged": 7 }
  ]
}
```

Idempotent on `(userId, slug)`: discover emits stable signature-derived
slugs, so re-applying the same candidate updates the existing row in
place and the reclassify pass re-stamps the same games under the same
name.
