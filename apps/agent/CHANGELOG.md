...
- Version 3 replay signatures reconstruct unit and building assignments for all
  ten logical control groups (0–9), including when the same unit is shared across
  groups. Opening assignment/recall order and timing distinguish habits such as
  assigning the starting Nexus to both 3 and 0. Invalid selections stay unknown.
- Retains saves to all eight camera bookmark slots (0–7), setup order/timing,
  and global/per-slot rhythm of observed returns to saved positions. Camera
  returns do not establish which physical key or input caused the movement.
- Automatic selection/group transactions generated by control-group steals are
  excluded from player action counts and rhythm and applied only once during
  membership reconstruction.

### Migration note

- Deploy the v3-capable API before distributing this agent. Run a full Re-sync
  to enrich existing target and candidate games; old v1/v2 signatures remain
  readable but cannot supply missing membership or bookmark evidence.
