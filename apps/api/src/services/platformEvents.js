...
class PlatformEventsService {
  ...
  async publish(userId, rawEvent) {
    const event = sanitizeEvent(rawEvent, this.now());
    ...
    if (this.io) {
      ...
      for (const token of tokens) {
        ...
        this.io.to(`overlay:${token.token}`).emit("overlay:platformEvent", event);
      }
    }
    return true;
  }
  ...
  async recentForUser(userId) {
    ...
    return rows
      .reverse()
      .map((row) => sanitizeEvent(row.event, this.now()))
      .filter(Boolean)
