...
This is static producer compatibility plus narrow immediate-training evidence,
not execution permission. Intersect with existing masks; preserve EOS externally.
...
SCHEMA = "protoss-action-eligibility-v1"
...
"""
    ...
    ui, available = _selection_evidence(frame, current, catalog)
    ...
    for function, rule, aliases in reviewed:
        ...
        for pointer, tag in enumerate(entity_tags):
            ...
            elif (rule.train and unit_types[tag] == rule.producer_id and ui["used"]
                  and ui["selected_tag"] == tag and not set(aliases).intersection(available)):
                blocked[pointer] = "selected_current_producer_ability_absent"
        ...
        if rule.train:
            ...
            if known:
                ...
                if cap - used < rule.supply:
                    reasons.append("current_HUD_insufficient_supply")
    ...
    return {"schema": SCHEMA, "function_mask": function_mask, "source_masks": sources,
            ...
            "observation_sha256": canonical_sha256(frame), "rules_sha256": canonical_sha256([asdict(rule) for rule in RULES]),
