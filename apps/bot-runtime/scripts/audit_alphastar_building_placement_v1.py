"""Read-only actual-checkpoint proof of the observed building overlap graph.
...
from scripts.alphastar_eligibility_bridge_v2 import (  # noqa: E402
    FUNCTION_INPUT, SOURCES_INPUT,
)
...
from scripts.preflight_action_eligibility_v2 import PREFERRED_SHA  # noqa: E402
...
from pluto_sc2.action_eligibility_v2 import build_action_eligibility  # noqa: E402
from pluto_sc2.building_placement_v1 import build_placement_masks  # noqa: E402
from scripts.alphastar_building_placement_bridge_v1 import (  # noqa: E402
    PLACEMENT_ADAPTER, WORLD_INPUT, CLASSES_INPUT, build_placement_bridge,
)
...
def add_placement(example, config, registry, masks=None):
    result = {**example, "inputs": dict(example["inputs"])}
    result["inputs"][WORLD_INPUT] = (np.ones((3, config.world_size**2), bool) if masks is None else
                                      np.asarray(masks["masks"], bool).reshape(3, config.world_size**2))
    result["inputs"][CLASSES_INPUT] = (np.full(len(registry), -1, np.int32) if masks is None else
                                        np.asarray(masks["function_classes"], np.int32))
    return result
...
def run(args):
    ...
    state = {"schema": "alphastar-building-placement-graph-audit-v1", "status": "running",
             "adapter": PLACEMENT_ADAPTER, "started_unix": time.time(), "pid": os.getpid(),
    ...
    try:
        ...
        if (preflight.get("schema") != "action-eligibility1201-cpu-preflight-v2"
                or preflight.get("checkpoint_sha256") != PREFERRED_SHA
                or preflight["status"] != "label_contract_passed" or preflight["teacher_labels_rejected"] != 0
        ...
        for path in (Path(__file__), ROOT / "scripts/alphastar_eligibility_bridge_v2.py",
                     ROOT / "scripts/alphastar_eligibility_bridge_v1.py",
        ...
        placement_report = Path(args.placement_preflight).resolve()
        state["source_and_input_hashes"][str(placement_report)] = sha256(placement_report)
        placement_cpu = json.loads(placement_report.read_text(encoding="utf-8"))
        if (placement_cpu.get("schema") != "building-world-saved-evidence-audit-v1"
                or placement_cpu.get("status") != "passed"
                or placement_cpu.get("checkpoint_sha256") != PREFERRED_SHA
                or placement_cpu.get("checkpoint_optimizer_updates") != 6089
                or placement_cpu.get("graph_ready") is not True
                or placement_cpu.get("building_teacher_count") != 28
                or placement_cpu.get("teacher_masks_rejected") != 0
                or placement_cpu.get("source_inputs_unchanged") is not True):
            raise TensorError("Placement CPU audit does not admit all28 original building targets")
        for path, digest in placement_cpu["source_and_input_hashes"].items():
            if path.startswith("C:\\") or path.startswith("C:/"):
                path = "/mnt/c/" + path[3:].replace("\\", "/")
            if sha256(path) != digest:
                raise TensorError("Placement preflight provenance changed")
            if path in state["source_and_input_hashes"] and state["source_and_input_hashes"][path] != digest:
                raise TensorError("Placement and eligibility provenance disagree")
            state["source_and_input_hashes"][path] = digest
        for path in (ROOT / "scripts/alphastar_building_placement_bridge_v1.py",
                     ROOT / "src/pluto_sc2/building_placement_v1.py"):
            state["source_and_input_hashes"][str(path)] = sha256(path)
        ...
        building_abilities = {entry["ability_id"] for entry in artifacts["registry"]
                              if entry["name"].startswith("Build_") and "world" in entry["args"]}
        building_identities = {identity(row) for row in rows if row["intent"].get("ability_id") in building_abilities}
        cpu_records = placement_cpu.get("records", [])
        if (len(building_identities) != 28 or len(cpu_records) != 28
                or {tuple(record["identity"]) for record in cpu_records} != building_identities):
            raise TensorError("Placement receipt differs from exact28 building identities")
        ...
        probes = [next(row for row in rows if row["intent"].get("ability_id") == 883),
                  next(row for row in rows if not row["intent"].get("source_tags", []))]
        ...
        def graph(eligible, training):
            ...
            example = add_placement(add_eligibility(example, config, registry), config, registry) if eligible else example
            builder = build_placement_bridge if eligible else build_capacity_bridge
        ...
        for row in probes:
            ...
            all_placement = build_placement_masks(row["frame"], registry, artifacts["catalog"])
            all_placement["masks"] = np.ones((3, config.world_size, config.world_size), bool)
            a, (a_loss, a_gradient) = legacy_train(teacher, True)
            b, (b_loss, b_gradient) = new_train(add_placement(
                add_eligibility(teacher, config, registry), config, registry, all_placement), True)
            ...
            d, _ = new_infer(add_placement(add_eligibility(observation, config, registry),
                                           config, registry, all_placement))
        ...
        for row in rows:
            ...
            outputs, _ = new_infer(add_placement(
                add_eligibility(observation, config, registry, eligibility), config, registry, placement))
            ...
            if not prediction["teacher_quarantined"]:
                teacher = tensorize_sample(row, registry, artifacts["unit_types"], config)
                teacher_outputs, _ = new_train(add_placement(add_eligibility(teacher, config, registry, eligibility), config, registry, placement))
...
def main():
    ...
    parser.add_argument("--placement-preflight", type=Path, required=True)
