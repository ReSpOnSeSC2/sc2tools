"""Version2 admission retains explicitly receipted empty views with zero examples.

Numerical model/loss/optimizer contract matches version1 for exact continuation.
"""
...
def load_dataset(dataset):
    ...
    empty_receipts = result.get("empty_perspectives", [])
    empties = {perspective_key(row): row for row in empty_receipts}
    actual_empties = {perspective_key(row) for row in raw if row.get("events") == []}
    if (len(empties) != len(empty_receipts) or actual_empties != set(empties)
            or (actual_empties and result.get("extraction_contract") != "own-macro-attempt-sequences-empty-views-v2")):
        raise ValueError("Empty-view metadata must match an explicit version2 extraction receipt")
    ...
    for row in raw:
        ...
        if identity in empties:
            receipt = empties[identity]
            if (receipt.get("reason") != "no_classified_own_macro_attempts"
                    or receipt.get("extraction_counts") != row.get("extraction_counts")
                    or any(receipt.get(key) != row[key] for key in ("replay_id", "player_id"))):
                raise ValueError("Empty-view receipt identity/reason/counts changed")
        record = normalize_record(row, allow_empty=identity in empties)
...
def normalize_record(record, *, allow_empty=False):
    ...
    if result["commands"] or not allow_empty:
        validate_record(result)
    elif result["race"] not in ("Protoss", "Terran", "Zerg") or result["partition"] not in ("train", "validation"):
        raise ValueError("Invalid empty-view race/partition metadata")
...
def run(args):
    ...
    status = dict(
        ...
        dataset_admission="empty-view-receipts-v2; metadata retained, no fabricated training/evaluation examples",
        empty_perspectives=[perspective_key(row) for row in records if not row["commands"]],
    ...
    try:
        for race in ("Protoss", "Terran", "Zerg"):
            ...
            result = dict(
                ...
                empty_training_perspectives=sum(not row["commands"] for row in train),
                empty_validation_perspectives=sum(not row["commands"] for row in valid),
