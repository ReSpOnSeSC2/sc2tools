"""Distinct bounded Protoss depth curriculum with frozen worker retention.

Restores the reviewed 3492-update parent exactly, then explicitly changes only
the learning rate and supervised loss weighting, adding parent KL on TRAIN
worker contexts only. Names classify TRAIN labels;
...
SCHEMA = "own-command-build-depth-v2"
CONTRACT = "causal-own-command-depth-worker-retention-v2"
...
CATEGORY_WEIGHTS = {"worker": 1, "building": 1.25, "army_production": 1.5,
                    "research": 1.25, "cancel": 1, "other": 1, "unknown": 1}
...
def weighted_imitation_loss(logits, predicted_wait, target, wanted_wait, weights):
    ...
    if (target.ndim != 1 or predicted_wait.shape != target.shape or wanted_wait.shape != target.shape
            ...
            or not bool(torch.isfinite(weights).all()) or bool(((weights < 1) | (weights > 1.5)).any())):
...
def worker_retention_loss(logits, teacher_logits, worker_mask):
    """KL(parent||student), T1, normalized by worker rows, not all batch rows."""
    if worker_mask.dtype != torch.bool or worker_mask.shape != (logits.shape[0],):
        raise ValueError("Worker mask must match TRAIN batch rows")
    if not bool(torch.isfinite(logits).all()):
        raise ValueError("Student logits must be finite")
    if not bool(worker_mask.any()):
        return logits.sum() * 0.
    if (teacher_logits is None or teacher_logits.shape != logits.shape
            or teacher_logits.requires_grad or not bool(torch.isfinite(teacher_logits).all())
            or not torch.equal(teacher_logits.eq(-1e9), logits.eq(-1e9))):
        raise ValueError("Teacher logits must be detached, finite and match student support")
    student_log_probs = torch.nn.functional.log_softmax(logits[worker_mask], dim=-1)
    teacher_probs = torch.nn.functional.softmax(teacher_logits[worker_mask].detach(), dim=-1)
    return torch.nn.functional.kl_div(student_log_probs, teacher_probs, reduction="none").sum(-1).mean()


def make_teacher(parent):
    # Constructing another GRU consumes RNG; isolate that construction so the
    # student's exact restored random streams stay at the selected parent state.
    with torch.random.fork_rng(devices=[]):
        teacher = base.BuildOrderPrior()
        teacher.load_state_dict(parent["parameters"], strict=True)
    teacher.requires_grad_(False)
    teacher.eval()
    verify_teacher(teacher, parent)
    return teacher


def verify_teacher(teacher, parent):
    if (teacher.training or any(p.requires_grad or p.grad is not None for p in teacher.parameters())
            or not exact(teacher.state_dict(), parent["parameters"])):
        raise ValueError("Frozen parent teacher mutated or acquired gradients")


def initial_teacher_proof(model, teacher, rows, vocabulary, categories, check):
    examples = [(r, i) for r in rows if r["partition"] == "train" and r["race"] == "Protoss"
                for i, command in enumerate(r["commands"]) if command["game_loop"] <= 480 * 22.4
                and categories[base.token_key(command)]["category"] == "worker"][:32]
    if not examples:
        raise ValueError("Worker-retention contract needs actual TRAIN worker examples")
    check()
    model.eval()
    tokens, times, lengths, _, _ = base.collate_examples([base.prefix_example(r, i, vocabulary) for r, i in examples])
    with torch.no_grad():
        actual, _ = model(tokens, times, lengths, len(vocabulary))
        reference, _ = teacher(tokens, times, lengths, len(vocabulary))
        kl = worker_retention_loss(actual, reference, torch.ones(len(examples), dtype=torch.bool))
    if not torch.equal(actual, reference) or abs(float(kl)) > 1e-6:
        raise ValueError("Initial teacher/student predictions or zero-KL proof failed")
    check()
    return {"train_worker_examples": len(examples), "logits_exact": True,
            "worker_kl": float(kl), "absolute_kl_tolerance": 1e-6, "optimizer_updates": 0}
...
def require_bounds(args):
    if (type(args.epochs) is not int or not 1 <= args.epochs <= 2
        ...
        raise ValueError("Depth run limited to2 epochs,900s,2CPU threads and unchanged480s horizon")
...
def run(args):
    ...
    if updates_per_epoch * args.epochs > MAX_UPDATES:
        raise ValueError("Depth curriculum exceeds756 new updates")
    ...
    teacher = make_teacher(parent)
    if not torch.equal(torch.get_rng_state(), parent["torch_rng"]):
        raise ValueError("Teacher creation changed restored student RNG")
    teacher_proof = initial_teacher_proof(model, teacher, train, vocabulary, categories, check)
    check()
    unchanged()
    ...
    recipe = {"schema": CONTRACT, "parent_checkpoint_sha256": PARENT_SHA,
        ...
        "loss": "sum(weight*per-example CE)/sum(weight) +0.1 mean(unweighted smoothL1(log1p wait)) +1.0 mean_worker_rows(KL(parent||student))",
        "teacher": {"checkpoint_sha256": PARENT_SHA, "frozen": True, "no_grad": True,
            ...
            "worker_free_batch": "differentiable zero; no teacher forward required",
            "timing_distillation": False},
    ...
    status = {"schema": SCHEMA, "status": "running", "pid": os.getpid(),
        ...
        "restore_proof": {"parameters_exact": True, "adam_exact_before_lr_change": True,
            ...
            "teacher_exact_frozen_no_grad": True, "teacher_creation_rng_preserved": True},
        "initial_teacher_proof": teacher_proof,
    ...
    def save_checkpoint(name, epoch, aliases, *, after_stop=False):
        ...
        value = dict(schema=SCHEMA, race="Protoss", parameters=model.state_dict(), optimizer=optimizer.state_dict(),
            ...
            teacher_reference={"checkpoint_sha256": PARENT_SHA, "frozen_exact": True, "no_grad": True})
        buffer = io.BytesIO()
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            model.train()
            command_sum, timing_sum, weight_sum, kl_sum, worker_count = 0., 0., 0., 0., 0
            exposures = Counter()
            for start in range(0, len(examples), 128):
                ...
                worker_mask = torch.tensor([info["category"] == "worker" for info in infos], dtype=torch.bool)
                teacher_logits = None
                if bool(worker_mask.any()):
                    with torch.no_grad():
                        teacher_logits, _ = teacher(tokens, times, lengths, len(vocabulary))
                kl = worker_retention_loss(logits, teacher_logits, worker_mask)
                loss = loss + kl
                ...
                count = int(worker_mask.sum())
                kl_sum += float(kl.detach()) * count
                worker_count += count
                weight_sum += mass
            ...
            record = {"epoch": epoch, "cumulative_epoch": 11 + epoch, "updates": updates,
                ...
                "events": len(examples),
                "training_weighted_loss": command_sum / weight_sum + .1 * timing_sum / len(examples) + kl_sum / max(1, worker_count),
                "training_weighted_command_ce": command_sum / weight_sum,
                "training_unweighted_timing_loss": timing_sum / len(examples),
                "training_worker_normalized_kl": kl_sum / max(1, worker_count),
                "training_worker_anchor_examples": worker_count,
                "teacher_exact_frozen_no_grad": True,
