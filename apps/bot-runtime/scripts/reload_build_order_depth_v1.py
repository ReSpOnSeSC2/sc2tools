"""Independent read-only restoration proof for the distinct Protoss depth prior.
...
CATEGORY_WEIGHTS = {"worker": 1, "building": 2, "army_production": 3, "research": 2,
                    "cancel": 1, "other": 1, "unknown": 1}
APPROVED_LOSS = "sum(weight*per-example CE)/sum(weight) +0.1 mean(unweighted smoothL1(log1p wait)) within each batch"
...
def verify_adam(checkpoint, model):
    ...
    require(type(updates) is int and PARENT_UPDATES <= updates <= PARENT_UPDATES + 1134, "Depth optimizer budget/count changed")
...
def verify_recipe(recipe):
    require(recipe["learning_rate"] == LEARNING_RATE and recipe["parent_learning_rate"] == .002
            ...
            and recipe["schema"] == "causal-own-command-depth-v1"
            ...
            and recipe["max_new_updates"] == 1134 and 1 <= recipe["epochs"] <= 3
...
def run(args):
    ...
    def check():
        ...
        require(time.monotonic() - started <= 300, "Read-only reload wall bound reached")

    check()
    status_bytes = (output / "status.json").read_bytes()
    status = json.loads(status_bytes)
    require(status.get("schema") == "own-command-build-depth-v1" and status.get("status") == "complete"
            ...
            and status.get("horizon_seconds") == 480 and 1 <= status.get("epochs_requested", 0) <= 3, "Depth run not complete or contract changed")
    ...
    expected_contract = {**parent["model_contract"], "training_contract": "causal-own-command-depth-v1",
    ...
    for index in range(len(saved["passes"]) + 1):
        ...
        require(checkpoint["schema"] == "own-command-build-depth-v1" and checkpoint["race"] == "Protoss"
                and checkpoint["model_contract"] == status["model_contract"]
                and checkpoint["model_contract"]["training_contract"] == "causal-own-command-depth-v1"
    ...
    report = {"schema": "protoss-depth-independent-reload-v1", "status": "passed", "source_inputs_unchanged": True,
