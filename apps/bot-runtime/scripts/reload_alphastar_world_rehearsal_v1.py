"""Independent observation-only reload of a completed world-rehearsal diagnostic."""
...
def require_candidate_schema(result):
    require(result.get('schema') == 'alphastar-world-rehearsal-candidate-v1'
            and result.get('diagnostic_mode') == 'world-rehearsal-opening50-retention40-other10-v1'
            and result.get('optimizer_namespace') == 'world-head-only-adam-v1'
            and result.get('ordinary_full_model_continuation_allowed') is False
            and result.get('checkpoint_restore_verified') is True
            and type(result.get('new_optimizer_updates')) is int
            and 0 < result['new_optimizer_updates'] <= 128
            and result.get('optimizer_updates') == 6089 + result['new_optimizer_updates']
            and result.get('objective_proof_sha256')
            and result.get('specialization_budget', {}).get('previously_spent') == 64
            and result.get('specialization_budget', {}).get('ceiling') == 256,
            'Require the explicit bounded world-rehearsal checkpoint namespace')
...
def run(args):
    ...
    require_candidate_schema(result)
    ...
    report = {'schema': 'world-rehearsal-independent-reload-v1', 'status': 'passed',
        'checkpoint_sha256': result['checkpoint_sha256'], 'optimizer_count': decoded['optimizer_updates'],
        'new_optimizer_updates': 0, 'game_launches': 0, 'objective_proof_sha256': result['objective_proof_sha256'], 'source_inputs_unchanged': True,
