...
from scripts.reload_build_order_depth_v1 import (
    APPROVED_LOSS, CATEGORY_WEIGHTS, PARENT_SHA, exact, independent_categories,
...
def test_recipe_rejects_weighted_timing_and_extra_updates():
    value = {"learning_rate": .0005, "parent_learning_rate": .002, "parent_checkpoint_sha256": PARENT_SHA,
        "parent_updates": 3492, "schema": "causal-own-command-depth-v1", "category_weights": CATEGORY_WEIGHTS,
        "loss": APPROVED_LOSS, "adam_moments_preserved": True, "horizon_seconds": 480, "max_new_updates": 1134,
        "epochs": 3, "threads": 2, "wall_seconds": 900, "ambiguous_unknown_weight": 1, "stasis_trap_weight": 1}
    verify_recipe(value)
    for key, changed in (("loss", "weighted command and timing"), ("max_new_updates", 1135),
...
def test_category_proof_cannot_learn_mapping_from_validation_or_other_race():
    ...
    assert [categories[f"{i}:0"]["weight"] for i in range(1, 6)] == [1, 1, 3, 1, 1]
