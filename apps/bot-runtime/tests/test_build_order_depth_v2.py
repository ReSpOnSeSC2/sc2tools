...
SPEC = importlib.util.spec_from_file_location("depth_trainer_v2", Path(__file__).parents[1] / "scripts/train_build_order_depth_v2.py")
...
def test_categories_use_train_protoss_only_and_conflicts_default_one():
    ...
    assert [mapping[f"{i}:0"]["weight"] for i in (10, 11, 12)] == [1, 1.25, 1.5]
...
@pytest.mark.parametrize("name,category,weight", [
    ("TrainProbe", "worker", 1), ("BuildNexus", "building", 1.25),
    ("TrainImmortal", "army_production", 1.5), ("WarpInZealot", "army_production", 1.5),
    ("ResearchBlink", "research", 1.25), ("UpgradeGroundWeapons1", "research", 1.25),
...
def test_weighted_ce_analytical_loss_and_gradient_keep_timing_unweighted():
    ...
    weights = torch.tensor([1., 1.25, 1.5])
    actual, _, _ = depth.weighted_imitation_loss(logits, waits, targets, wanted, weights)
    manual = (torch.nn.functional.cross_entropy(logits, targets, reduction="none") * weights).sum() / weights.sum()
...
@pytest.mark.parametrize("key,value", [("epochs", 3), ("epochs", True), ("wall_seconds", 901),
                                      ("threads", 3), ("horizon_seconds", 481)])
def test_hard_resource_and_horizon_bounds(key, value):
    args = SimpleNamespace(epochs=2, wall_seconds=900, threads=2, horizon_seconds=480)
...
def test_stop_prevents_any_run_output(tmp_path):
    ...
    args = SimpleNamespace(dataset=dataset, parent_run=parent, output=output,
                           epochs=2, wall_seconds=900, threads=2, horizon_seconds=480)
    with pytest.raises(InterruptedError):
        depth.run(args)
...
def test_worker_kl_direction_normalization_and_gradients():
    teacher = torch.tensor([[0., 1., 2., -2.], [5., -1., -2., -3.], [-1., .2, 1., 3.]])
    student = torch.tensor([[1., 0., 2., -1.], [-1., 3., .2, 0.], [2., .5, 0., 1.]], requires_grad=True)
    mask = torch.tensor([True, False, True])
    actual = depth.worker_retention_loss(student, teacher, mask)
    parent_probs = torch.softmax(teacher[mask], -1)
    manual = (parent_probs * (torch.log_softmax(teacher[mask], -1) - torch.log_softmax(student[mask], -1))).sum(-1).mean()
    ...
    expected[mask] = (torch.softmax(student.detach()[mask], -1) - parent_probs) / 2
    assert torch.allclose(gradient, expected, atol=1e-7)
    assert torch.equal(gradient[1], torch.zeros(4))
    assert teacher.grad is None


def test_initial_equal_teacher_has_zero_kl_and_gradient_and_no_worker_is_zero():
    teacher = torch.tensor([[-1e9, -1e9, 2., 1.], [-1e9, -1e9, 1., 4.]])
    student = teacher.clone().requires_grad_()
    loss = depth.worker_retention_loss(student, teacher, torch.ones(2, dtype=torch.bool))
    assert abs(float(loss.detach())) < 1e-7
    assert torch.allclose(torch.autograd.grad(loss, student)[0], torch.zeros_like(student), atol=1e-7)
    zero = depth.worker_retention_loss(student, None, torch.zeros(2, dtype=torch.bool))
    assert zero.requires_grad and float(zero.detach()) == 0.
    assert torch.equal(torch.autograd.grad(zero, student)[0], torch.zeros_like(student))


def test_teacher_creation_preserves_rng_and_never_trains_or_mutates():
    parent = trained_parent()
    torch.set_rng_state(parent["torch_rng"])
    before = torch.get_rng_state().clone()
    teacher = depth.make_teacher(parent)
    assert torch.equal(torch.get_rng_state(), before)
    depth.verify_teacher(teacher, parent)
    assert not teacher.training and all(not p.requires_grad and p.grad is None for p in teacher.parameters())
    with torch.no_grad():
        next(teacher.parameters()).view(-1)[0] += 1
    with pytest.raises(ValueError, match="teacher"):
        depth.verify_teacher(teacher, parent)


def test_teacher_logits_requiring_gradient_are_rejected():
    with pytest.raises(ValueError):
        depth.worker_retention_loss(torch.zeros(2, 4, requires_grad=True),
                                    torch.zeros(2, 4, requires_grad=True), torch.ones(2, dtype=torch.bool))


def test_teacher_and_student_masked_support_must_match():
    teacher = torch.tensor([[-1e9, -1e9, 2., 1.]])
    with pytest.raises(ValueError, match="support"):
        depth.worker_retention_loss(torch.zeros(1, 4), teacher, torch.ones(1, dtype=torch.bool))
