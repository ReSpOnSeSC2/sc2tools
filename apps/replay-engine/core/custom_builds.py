...
def _is_v1(data: Dict[str, Any]) -> bool:
    ...
    Stage settings-pr1i: tightened so that ANY numeric ``version`` key
    disqualifies a file from being treated as v1, even if its builds
    have a ``rules`` field. The Express side bumped to v3 (which also
    uses per-build ``rules``) and the old heuristic was destructively
    "migrating" v3 files back to an empty v2 every time this loader
    ran. The legitimate v1 file (Spawning-Tool-style) has NO version
    key at all -- only ``target``/``rules`` distinguishes it -- so we
    keep matching that case while leaving v2/v3 alone.
    """
...
def load_custom_builds_v2() -> Dict[str, Any]:
    ...
    Stage settings-pr1i: accept v2 OR newer (v3+) without rewriting.
    The Express backend (``routes/custom_builds_helpers.js``) is the
    canonical writer and is currently at v3; this Python module is
    read-only for v2+ files and only ever WRITES on a v1->v2 migration
    of a legitimate Spawning-Tool-format file (no ``version`` key).
    """
    ...
    if isinstance(version, int) and version >= SCHEMA_VERSION:
        # v2 or newer (v3 from Express, future schemas) -- pass through
        # unchanged so we never destructively downgrade.
        return raw
    LOGGER.warning(
        "custom_builds.unexpected_version version=%s", version
    )
    return _empty_v2_file()
