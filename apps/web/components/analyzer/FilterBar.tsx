 ...
 * "all on" (no constraint) so a single-region grinder sees nothing
 ...
 * deselected at least one — full set stays as the no-op zero-byte URL.
 *
...
function RegionToggleRow() {
  ...
  const toggle = (code: string) => {
    ...
    // Empty deselect-all is meaningless (returns zero rows); treat it
    // as a re-selection of every region so the user can't accidentally
    // blank the dashboard with two taps.
    if (next.size === 0) {
      setFilters({ ...filters, regions: undefined });
      return;
    }
    // All-on is the default; send no param so existing bookmarks keep
    // a clean URL.
