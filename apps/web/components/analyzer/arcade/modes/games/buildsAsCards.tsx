...
async function generate(input: GenerateInput): Promise<GenerateResult<Q>> {
  ...
  // The bundled catalog is non-empty, so an empty universe means we
  // truly have no data to work with. Probe path (useEligibleDailyPicks)
  // uses an empty dataset to ask "is this mode eligible today?" — we
  // tell it no when only the catalog stubs would render, so a brand-
  // new account doesn't get Builds-as-Cards picked as their Daily.
