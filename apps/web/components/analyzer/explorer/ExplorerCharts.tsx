    ...
    <div role="img" aria-label={`${weighted ? "Player-weighted win rate" : "Win rate"} and game count by group. Select Data for exact values and accessible game links.`} className="h-72 min-w-0 w-full sm:h-80">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={values} margin={{ top: 16, right: 0, left: -20, bottom: 6 }} onClick={(state) => { const row = state?.activePayload?.[0]?.payload as ExplorerRow | undefined; if (row?.games) onSelect(row); }} accessibilityLayer>
          ...
          {comparison ? <Bar yAxisId="rate" dataKey="ratePct" name="Win rate" radius={[6, 6, 0, 0]} maxBarSize={104} cursor="pointer" isAnimationActive={false}>{values.map((row, index) => <Cell key={row.key} fill={index === 0 ? COLORS.accent : COLORS.secondary} fillOpacity={0.8} />)}</Bar> : <>
            <Bar yAxisId="games" dataKey="games" name="Games" fill={COLORS.accent} fillOpacity={0.17} radius={[4, 4, 0, 0]} maxBarSize={60} cursor="pointer" isAnimationActive={false} />
            <Line yAxisId="rate" dataKey="ratePct" name="Win rate" stroke={COLORS.accent} strokeWidth={2.5} connectNulls={false} dot={{ r: 4, fill: "rgb(var(--bg-surface))", strokeWidth: 2 }} activeDot={{ r: 6, cursor: "pointer" }} isAnimationActive={false} />
          </>}
        </ComposedChart>
      </ResponsiveContainer>
    </div>
