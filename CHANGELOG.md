...
- **Exact barcode group and camera habits** — v3 signatures match unit/building
  ...
  first-save timing, and observed return rhythm globally and per slot. Expanded
  comparisons show actual opening examples and repeated habits across games.
  Missing or poorly decoded membership cannot become a strong matching signal.
  Ships in agent **0.16.5** (`agent-v0.16.5`); existing games need a full Re-sync.
...
- **Control-group steals no longer inflate behavioral input counts** — the
  parser recognizes the automatic buffer updates generated by a steal, avoids
  applying their removal twice, and excludes them from player recall rhythm.
