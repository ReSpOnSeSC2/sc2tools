...
(function () {
    ...
    const EVENT_REGISTRY = {
        ...
        // Live Game Bridge envelope from the desktop agent (see
        // sc2tools_agent.live + the overlay backend's
        // POST /api/agent/live mapping). Carries phase + opponent
        // (with optional Pulse profile) so widgets can render
        // progressively from MATCH_LOADING onwards. The backend
        // ALSO maps the same envelope to opponentDetected /
        // scoutingReport for widgets that pre-date this handler;
        // having both doesn't double-render because each widget
        // owns its own DOM root, and the dedupe in voice-readout
        // suppresses repeat readouts of identical payloads.
        liveGameState: {
            slot: 'live-bridge',
            defaultDurationMs: 60000,
            render(p) {
                const phase = String(p.phase || '').toLowerCase();
                // IDLE / MENU clear the persistent opponent + scout
                // widgets so a stale payload doesn't sit on screen
                // between matches.
                if (phase === 'idle' || phase === 'menu') {
                    if (els.opponent && els.opponent.root) {
                        try { hide(els.opponent.root); } catch (_) {}
                    }
                    if (els.scout && els.scout.root) {
                        try { hide(els.scout.root); } catch (_) {}
                    }
                    return;
                }
                const opp = (p.opponent && typeof p.opponent === 'object') ? p.opponent : {};
                const profile = (opp.profile && typeof opp.profile === 'object') ? opp.profile : {};
                const oppName = opp.name || null;
                const oppRace = opp.race || profile.top_race || null;
                const oppMmr = Number.isFinite(profile.mmr) ? profile.mmr : null;
                // Render the persistent opponent widget directly so
                // it lights up at MATCH_LOADING (T+0) instead of
                // waiting for the duplicated opponentDetected event
                // to flow through the queue.
                if (els.opponent && els.opponent.root) {
                    safeText(els.opponent.details, oppName || 'Looking up opponent…');
                    safeText(els.opponent.mmr, oppMmr != null ? `${oppMmr} MMR` : '');
                    safeText(els.opponent.race, oppRace || '');
                    setIconSlot(els.opponent.icon, Icons.raceIcon(oppRace), oppRace);
                    if (els.opponent.record) {
                        // No H2H yet — the post-game replay parse
                        // will overwrite this with the cloud-derived
                        // record via the existing opponentDetected
                        // handler.
                        safeText(els.opponent.record, oppName ? '' : 'Looking up opponent…');
                    }
                    if (els.opponent.winrate) safeText(els.opponent.winrate, '');
                    if (oppName || oppRace) show(els.opponent.root);
                }
                // Same skeleton-then-enrich path for the scouting card.
                if (els.scout && els.scout.root) {
                    setIconSlot(els.scout.raceIcon, Icons.raceIcon(oppRace), oppRace);
                    safeText(els.scout.name, oppName || 'Looking up opponent…');
                    if (els.scout.matchup) {
                        els.scout.matchup.style.display = 'none';
                    }
                    if (els.scout.record) {
                        safeText(els.scout.record,
                            oppMmr != null ? `${oppMmr} MMR` : '');
                    }
                    if (els.scout.rivalRow) els.scout.rivalRow.style.display = 'none';
                    if (els.scout.recentRow) els.scout.recentRow.style.display = 'none';
                    if (els.scout.ansRow) els.scout.ansRow.style.display = 'none';
                    if (els.scout.cheeseRow) els.scout.cheeseRow.style.display = 'none';
                    show(els.scout.root);
                }
            },
            // Persistent during the match — no-op hide so the
            // widgets stay on screen until the next idle/menu phase
            // clears them.
            hide() { /* persistent */ }
        },
