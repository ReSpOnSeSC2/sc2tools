...
class OpponentStrategyDetector(BaseStrategyDetector):
    ...
    def get_strategy_name(self, race: str, enemy_events: List[Dict], matchup: str = "vs Any") -> str:
        ...
        # events so the same building isn't double-counted (init +
        # born), and sorted-index access (e.g. ``nexus_times[2]``)
        ...
        # filter (init+born only, excluding "morph"). Alias to the
        # canonical start-time count so the two helpers can't drift.
        ...
        # --- TERRAN ---
            ...
            # Ghost Rush: tighten to 5:30 -- standard Bio macro often
            ...
            # BC Rush: require a real Battlecruiser by 7:30 off a
            # Fusion Core started by 5:30 -- a committed Starport-into-
            # FC opener. Mirrors the fix in the modular opponent
            # detector.
